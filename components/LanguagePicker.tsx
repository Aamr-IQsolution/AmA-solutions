...
const LanguagePicker: React.FC<LanguagePickerProps> = ({ onSelect }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-[#050510] flex items-center justify-center p-6 overflow-hidden">
      ...
      <div className="max-w-4xl w-full relative z-10">
        <div className="text-center mb-16 animate-in fade-in slide-in-from-top duration-1000">
          <div className="inline-block relative mb-8 group">
            ...
            <img src={config.logo} alt="Logo" className="relative h-24 w-24 md:h-32 md:w-32 rounded-full border-2 border-white/20 shadow-2xl mx-auto object-cover bg-black" />
          </div>
          <h1 className="text-4xl md:text-6xl font-black text-white mb-4 ama-text-gradient uppercase tracking-tighter">
            Choose Your Language
          </h1>
          <p className="text-gray-400 text-lg font-medium">إختر لغتك المفضلة للمتابعة</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 md:gap-8">
          {languages.map((lang, idx) => (
            <button
              ...
              className="group relative glass-card p-8 rounded-[40px] border border-white/5 hover:border-cyan-500/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-cyan-500/10 animate-in fade-in slide-in-from-bottom duration-700"
              style={{ animationDelay: `${idx * 150}ms` }}
            >
              <div className="flex flex-col items-center gap-6">
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden border-4 border-white/10 group-hover:border-cyan-400 transition-colors shadow-xl bg-black/20">
                ...
                <div className="text-center">
                  <h3 className="text-2xl font-black text-white mb-2 group-hover:ama-text-gradient transition-colors">
                    {lang.label}
                  </h3>
